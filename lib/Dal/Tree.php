...
class Tree
{
	...
	public static function Insert($m,$Conn=null)
	{
    	...
    	if(!$m->_TreeId->Changed)
    	   $m->TreeId($IsMyCat?'next value for MYCATSEQ_TREE':-1);
	...
	public static function ForUpdate($TreeId,$Conn)
	{
		...
		return Data::Execute("select * from ".self::TableName()." where TREE_ID= ? for UPDATE",[$TreeId],$Conn);
	}

	public static function Del($TreeId,$Conn=null)
	{
	    $TreeId=intval($TreeId);
		$Sql="delete from ".self::TableName()." where TREE_ID= ?";
		return Data::Execute($Sql,[$TreeId],$Conn);
	}
	...
	public static function Row($TreeId,$SqlField = "*",$Whare = "" ,$Conn=null,$ForUpdate=false)
	{
	    $row=null;
	    $TreeId=intval($TreeId);
		...
		if($TreeId>0)
		{
			$Sql="select $SqlField from ".self::ViewName()." where TREE_ID = '{$TreeId}' limit 1";
			if($Conn && $ForUpdate)
		      $Sql=$Sql.' for UPDATE';
	...
	public static function Model($TreeId,$Where = "",$Conn=null,$ForUpdate=false)
	{
	    $m=false;
	    $dr=self::Row($TreeId,"*",$Where,$Conn,$ForUpdate);
	    if($dr)
	    {
	       	$m=new \Model\Tree();
	...
	public static function GetListByIndex($_LastID,$_PageSize,&$_RecordCount,$_Fields,$_Where,$_OrderBy,$Conn=null)
	{
		...
		if(is_array($_Where))
		{
		    ...
		    if(Fram::IsNotEmpty($_Where[0]))
		      $_Where[0]="where {$_Where[0]}";
    		$_Where[0].=(strlen($_Where[0])>0)?" and TREE_ID>{$_LastID}":"TREE_ID>{$_LastID}";
    		$_RecordCount=Data::Execute_Column("select count(1) from ".self::TableName()." {$_Where[0]}",$_Where[1],$Conn);
		...
		{
		    $_Where.=($_Where && strlen($_Where)>0)?" and TREE_ID>{$_LastID}":"TREE_ID>{$_LastID}";
