...
class Role
{
	...
	public static function Insert($m,$Conn=null)
	{
    	...
    	if(!$m->_RoleId->Changed)
    	   $m->RoleId($IsMyCat?'next value for MYCATSEQ_ROLE':-1);
	...
	public static function ForUpdate($RoleId,$Conn)
	{
		...
		return Data::Execute("select * from ".self::TableName()." where ROLE_ID= ? for UPDATE",[$RoleId],$Conn);
	}

	public static function Del($RoleId,$Conn=null)
	{
	    $RoleId=intval($RoleId);
		$Sql="delete from ".self::TableName()." where ROLE_ID= ?";
		return Data::Execute($Sql,[$RoleId],$Conn);
	}
	...
	public static function Row($RoleId,$SqlField = "*",$Whare = "" ,$Conn=null,$ForUpdate=false)
	{
	    $row=null;
	    $RoleId=intval($RoleId);
		...
		if($RoleId>0)
		{
			$Sql="select $SqlField from ".self::ViewName()." where ROLE_ID = '{$RoleId}' limit 1";
			if($Conn && $ForUpdate)
		      $Sql=$Sql.' for UPDATE';
	...
	public static function Model($RoleId,$Where = "",$Conn=null,$ForUpdate=false)
	{
	    $m=false;
	    $dr=self::Row($RoleId,"*",$Where,$Conn,$ForUpdate);
	    if($dr)
	    {
	       	$m=new \Model\Role();
	...
	public static function GetListByIndex($_LastID,$_PageSize,&$_RecordCount,$_Fields,$_Where,$_OrderBy,$Conn=null)
	{
		...
		if(is_array($_Where))
		{
		    ...
		    if(Fram::IsNotEmpty($_Where[0]))
		      $_Where[0]="where {$_Where[0]}";
    		$_Where[0].=(strlen($_Where[0])>0)?" and ROLE_ID>{$_LastID}":"ROLE_ID>{$_LastID}";
    		$_RecordCount=Data::Execute_Column("select count(1) from ".self::TableName()." {$_Where[0]}",$_Where[1],$Conn);
		...
		{
		    $_Where.=($_Where && strlen($_Where)>0)?" and ROLE_ID>{$_LastID}":"ROLE_ID>{$_LastID}";
