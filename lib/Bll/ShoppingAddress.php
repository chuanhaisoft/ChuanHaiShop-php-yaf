...
class ShoppingAddress
{
	...
	{
		return \Dal\ShoppingAddress::Insert($m,$Conn);
	}

	public static function Update($m,$Whare='',$Conn=null,$_IfRowCount=true)
	{
		return \Dal\ShoppingAddress::Update($m,$Whare,$Conn,$_IfRowCount);
	}
	
	public static function UpdateWhere($m,$Whare,$Conn=null,$_IfRowCount=true)
	{
		return \Dal\ShoppingAddress::UpdateWhere($m,$Whare,$Conn,$_IfRowCount);
	}
	
	public static function ForUpdate($Id,$Conn)
	{
		return \Dal\ShoppingAddress::ForUpdate($Id,$Conn);
	}

	public static function Del($Id,$Conn=null,$Whare='')
	{
		return \Dal\ShoppingAddress::Del($Id,$Conn,$Whare);
	}

	public static function DelRows($IDS,$Conn=null)
	{
		return \Dal\ShoppingAddress::DelRows($IDS,$Conn);
	}

	public static function DelWhere($w_arr,$Conn=null)
	{
		return \Dal\ShoppingAddress::DelWhere($w_arr,$Conn);
	}
	
	public static function Row($Id = -1,$Whare = "",$Fields = "*",$Conn=null,$ForUpdate=false)
	{
	    return \Dal\ShoppingAddress::Row($Id,$Fields,$Whare,$Conn,$ForUpdate);
	}
	...
	public static function Column($Id,$SqlField='*',$Whare='',$Conn=null)
	{
	    ...
	    if(Fram::CheckNum($Id))
	        $Whare=["ID=?",[$Id]];
	    return \Dal\ShoppingAddress::Column($SqlField,$Whare,$Conn);
	}
	
	public static function Model($Id,$Whare = "",$Conn=null,$ForUpdate=false)
	{
	    return \Dal\ShoppingAddress::Model($Id,$Whare,$Conn,$ForUpdate);
	}
	...
	public static function GetList($_PageNum,$_PageSize,$_Where="",&$_RecordCount=0,$_Fields="",$_OrderBy="",$Conn=null)
	{
	    if(!$_OrderBy || $_OrderBy=='')
	        $_OrderBy='ID desc';
	    return \Dal\ShoppingAddress::GetList($_PageNum,$_PageSize,$_RecordCount,$_Fields,$_Where,$_OrderBy,$Conn);
	}
	//index列表
	public static function GetListByIndex($_LastID,$_PageSize,$_Where="",&$_RecordCount=0,$_Fields="",$_OrderBy="",$Conn=null)
	{
	    if(!$_OrderBy || $_OrderBy=='')
	        $_OrderBy='ID desc';
	    return \Dal\ShoppingAddress::GetListByIndex($_LastID,$_PageSize,$_RecordCount,$_Fields,$_Where,$_OrderBy,$Conn);
	}
}